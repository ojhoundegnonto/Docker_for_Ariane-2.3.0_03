...
%% Number of particles %%
%%%%%%%%%%%%%%%%%%%%%%%%%%
nb_traj = size(traj_temp,2);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Colorbar for temp data %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c_temp=jet;
szc=size(c_temp,1);
...
delta_temp= ( max_temp - min_temp) / (szc-1);

ind_temp = ones(size(traj_temp(:,:)),'int32');

ind_temp(:,:) = int32( (traj_temp(:,:) - min_temp)/delta_temp) + 1;
...
for j=1:nb_traj
  for i=1:sz_traj_lon
    if ( m_lldist(traj_lon_plot(i:i+1,j), traj_lat_plot(i:i+1,j)) < 2000. )
	  ...
	  'LineWidth', 2,'Color', c_temp(ind_temp(i,j),:));
    end
  end
end
...
title({'Particle trajectories - Temp'}, 'fontweight', 'b');
