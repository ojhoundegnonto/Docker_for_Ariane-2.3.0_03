...
%%%%%%%%%%%%%%%%%%%%%%%%%%
nb_traj = size(traj_dens,2);

%%%%%%%%%%%%%%%%%%%%%%%%%%%%
%% Colorbar for dens data %%
%%%%%%%%%%%%%%%%%%%%%%%%%%%%
c_dens=jet;
szc=size(c_dens,1);
...
delta_dens= ( max_dens - min_dens) / (szc-1);

ind_dens = ones(size(traj_dens(:,:)),'int32');

ind_dens(:,:) = int32( (traj_dens(:,:) - min_dens)/delta_dens) + 1;
...
for j=1:nb_traj
  for i=1:sz_traj_lon
    if ( m_lldist(traj_lon_plot(i:i+1,j), traj_lat_plot(i:i+1,j)) < 2000. )
	  ...
	  'LineWidth', 2,'Color', c_dens(ind_dens(i,j),:));
    end
  end
end
...
title({'Particle trajectories - Dens'}, 'fontweight', 'b');
xlabel('longitude', 'fontweight', 'b');
ylabel('latitude', 'fontweight', 'b');

%% save the figure
print -dtiff traj_dens.tif;
